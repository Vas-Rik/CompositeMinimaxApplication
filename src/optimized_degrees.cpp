...
std::vector<int> compute_min_multdepth_update(RR alpha, RR epsilon, int depth, long maxdeg, bool is_comp)
{
	...
	RR temp;
	long mintime;
	long t_max=300, n_max=30;	// maxdeg should be odd!
	...
	for(long deg=3; deg<=maxdeg; deg+=2)
	{
	...
	vector<vector<vector<long>>> time(maxdeg+1, vector<vector<long>>(n_max+1, vector<long>(n_max+1,0)));
	RR expx;
	vector<size_t> num(maxdeg+1,0);
	for(long int deg=3; deg<=maxdeg; deg+=2) E[deg] >> num[deg];		// number of pairs
	for(long int deg=3; deg<=maxdeg; deg+=2)
	{
		RR temp;
...
//			cout << a << ": " << m << ", " << n << " Round" << endl;
	...
	lt = V[mintime][depth];
	// cout << "test1 " << endl;
	std::vector<int> resultList; // Vector to store the values
    // cout << "test2 " << endl;
	for(iter = lt.begin(); iter != lt.end(); iter++) {
        resultList.push_back(*iter); // Add value to resultList
    }    
    // cout << "test3 " << endl;
    // Optionally print the resultList to verify
...
std::vector<int> compute_min_multdepth(RR alpha, RR epsilon, long maxdeg, bool is_comp){
	...
	long m_max=70, n_max=40;	// maxdeg should be odd!
	...
	RR expx;
	vector<size_t> num(maxdeg+1,0);
	for(long deg=3; deg<=maxdeg; deg+=2) E[deg] >> num[deg];		// number of pairs is stored in each text file "En.txt"
	for(long deg=3; deg<=maxdeg; deg+=2)
	{
		RR temp;
		for(size_t i=0; i<num[deg]; i++) 
...
//			cout << a << ": " << m << ", " << n << " Round" << endl;
			...
			// m>=2 and n>=2 case
				...
				for(long k=1; mult(2*k+1) <= m && dep(2*k+1) <= n && 2*k+1<=maxdeg; k++) {	
	...
	for(int dep = mindep; dep <= mindep+10; dep++)
	{
		...
		if(is_comp == true)	cout << "depth 1: " << dep << endl;
