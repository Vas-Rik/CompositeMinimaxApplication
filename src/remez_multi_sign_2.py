...
def remez(func, n_degree: int, D: [(Decimal, Decimal)], max_iter: int = 10):
    ...
    max_for = find_zeros(temp_first_derivative, mpf(2 ** (-6.993709998)), 1)
    return coeffs_list, max(errors), max_for
...
alpha = float(6.993709998)
epsilon = mpf(2 ** (-alpha))

approximation_param = mpf(2 ** (1-alpha))

D = [(mpf(-1), mpf((-1) + epsilon)), (mpf(1 - epsilon), mpf(1))]
...
for degree in degrees:
    ...
    poly_coeffs, max_error, max_for = remez(function, degree, D, max_iter=10)
    D = [(mpf(-1), mpf(-1 + max_error)), (mpf(1 - max_error), mpf(1))]
    coeffs_list.append(poly_coeffs)
    # print(D)
...
y_approx = x
y_max_for = max_for
for poly_coeffs in coeffs_list:
    y_approx = np.polyval(poly_coeffs[::-1], y_approx)
    y_max_for = np.polyval(poly_coeffs[::-1], y_max_for)

y_exact_for = np.array([function(x_i) for x_i in max_for])


err_list = abs(y_max_for - y_exact_for)
#print("1: ", err_list)
err = max(err_list)
#print("2: ", err)
alph = math.log2(err)
print("alpha : ", alph)
print("1 - alpha : ", 1 + alph)
